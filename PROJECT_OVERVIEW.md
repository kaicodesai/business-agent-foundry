...
**2026-05-05 growth focus:** For the next 30 days, outbound is narrowed to US
...
- **[PA] Lead Generation** (YO3f5CL9bYbLTBgw) - 24 nodes, Apollo.io paid plan - patched 2026-05-08 so Apollo no longer treats generic `coach` as a health/wellness match; keyword rotation now uses wellness-specific coaching phrases and post-search filtering requires a real wellness signal before reveal. Same-day follow-up patch preserves the vetted pre-reveal company/industry/team-size when Apollo bulk reveal returns a different organization, preventing mismatches like a wellness email being written under a hospital/current-employer company; follow-up hardening also prevents mismatched revealed orgs from leaking their industry/team size into the selected company. Reliability hardening added retry/backoff to Apollo/Airtable HTTP nodes, reveal-specific error handling, pre-reveal dedup fallback, escaped post-reveal email lookup, paired/key-based dedup preparation, post-success keyword rotation, and removed a dead connection stub. Patched 2026-05-07 so Apollo fetch failures route through `Handle Apollo Error1` instead of leaving that recovery node orphaned. Patched 2026-05-05 for the 30-day US health/wellness 5-20 ICP. Reveal credits are protected by pre-reveal Airtable checks against `apollo_person_id`, LinkedIn URL, and email. Apollo bulk reveal uses stored `pa-apollo-io`; run notes log page/reveal/pre-seen counts.
...
- **[PA] Typeform Lead Qualification** (kXxN7O77ongTMwKG) — 14 nodes, active; Typeform submission → answer extraction → dedup → Prospects write → OpenRouter `qwen/qwen3.6-flash` score → Kai notification for Grade A/B leads at kai@phoenixautomation.ai. Patched 2026-05-07: scoring now matches Website Chatbot's new inbound ICP balance (wellness coaching preferred, non-wellness SMBs still valid, A for roughly 50-100 people plus clear automation intent; B for plausible borderline review). Duplicate lookup now uses a valid Airtable formula, create/update nodes preserve `Extract Answers`, and final score update writes `prospect_name`, `client_slug`, `source`, `lead_source`, and `sourced_at`. Existing matched Typeform records were backfilled for missing names. Parser patched for OpenRouter `choices[0].message.content`; latest safe smoke PASS execution 2620.
...
- **[PA] Scoping Agent** (E24KwVMam1e8bbjT) - 19 nodes, active; reads/writes **Prospects table**, uses OpenRouter `qwen/qwen3.6-max-preview` with `deepseek/deepseek-v4-pro` fallback, writes Airtable-safe JSON, maps service_tier to valid Prospects values, normalizes tools_required text, sets `project_status=scope_review`, and has corrected loop branch wiring. Patched 2026-05-01: added `When Executed by Scoping Notifier` Execute Sub-workflow Trigger so Scoping Notifier no longer calls `/scope-call` by HTTP. Patched 2026-05-06: `Prepare Client Data` derives `client_slug` from company/prospect name when missing, `Set Status to Scoping` writes the slug and sets `outreach_status=replied`, and `Write Scope to Airtable` persists the slug with generated scope fields. Public/manual Scope Call webhook and Scope Call Form remain available.
- **[PA] Scoping Agent** reliability patch 2026-05-08: primary OpenRouter call now uses `max_tokens=1200`, prompts directly for Airtable-valid service tiers (`starter-build`, `growth-package`, `scale-retainer`), and routes failed/empty primary model output through a `deepseek/deepseek-v4-pro` fallback before parsing. External HTTP/email nodes now have retry/backoff where appropriate.
- **[PA] Scoping Agent** polish patch 2026-05-08: future scoped prospects write `outreach_status=converted` using Airtable `typecast=true`, keeping true outreach replies separate from conversion state while still suppressing follow-up sends. AI output now asks for distinct `scope_summary` and fuller `scope_of_work`; the 2-hour poll also recovers stale `scoping` records with blank `scope_summary` after 1 hour.
- **[PA] Scope Approval** (UB6ZdrnYpJlYfxD4) - 12 nodes, active; reads/writes Prospects table, locks approved scope, saves `proposal_draft` to Airtable as source of truth, creates a ClickUp Lead Management review task in parallel, creates a real Gmail draft addressed to the prospect via `Gmail account 2`, and emails Kai at kai@phoenixautomation.ai. Patched 2026-05-08 with an idempotency gate: duplicate/preview/re-clicked approve links now return an "already handled" browser response without regenerating proposals, Gmail drafts, ClickUp tasks, or owner emails. Patched 2026-05-06: proposal generation now produces a cleaner client-ready email-style draft, uses canonical `kai@phoenixautomation.ai`, forbids exact invented pricing, requires owner-set price placeholder by tier range, discloses client-owned software/API costs separately, and sends Kai a professional "proposal draft created in Gmail" notification instead of rendering the raw proposal body in email.
- **[PA] Scope Approval** polish patch 2026-05-08: added `/reject-scope?client_slug=X` path that clears generated scope/proposal fields and returns the prospect to `call_complete` for regeneration after notes are edited. Scoping Agent review email now includes both Approve Scope and Reject Scope links. Kai's proposal-created email now surfaces whether the ClickUp proposal review task ID was confirmed.
...
- **[PA] Scoping Notifier** (nXXsF4E1BPWIS62r) — 17 nodes, active; webhook-first notifier for Airtable matched-record automations, with hourly fallback poll. Notifies Kai at kai@phoenixautomation.ai when a prospect has `project_status=call_complete`, blank `scoping_notified_at`, and non-empty `call_notes`; exposes GET /trigger-scoping for browser-triggered handoff. Patched 2026-05-08: browser handoff now marks `scoping_notified_at` after calling Scoping Agent, and external HTTP/email nodes have retry/backoff; `Mark Notified` is best-effort so a timestamp write failure does not block browser response. Patched 2026-05-01: browser handoff now calls Scoping Agent via native Execute Sub-workflow (`waitForSubWorkflow=false`) instead of webhook-to-webhook, while keeping Airtable/browser inbound webhooks intact.
...
## Workflow Linkage Map — 2026-05-01
- External/public entry webhooks remain webhooks: Website Chatbot (`/website-chatbot`), Typeform Lead Qualification (`/typeform-intake`), Onboarding payment confirmation (`/payment-confirmed`), Scope Approval browser approval (`/approve-scope`), Scoping Notifier browser/Airtable entrypoints (`/trigger-scoping`, `/scoping-notifier-airtable`), Morning Brief inbound (`/morning-brief`), and Outreach manual control (`/outreach-manual-trigger`).
- Internal workflow-to-workflow calls now use native n8n sub-workflows: Scoping Notifier → Scoping Agent (`Execute Sub-workflow`, no wait, keeps browser response fast) and Workflow Builder Agent → QA Agent (`Execute Sub-workflow`, waits for QA verdict).
- No remaining same-instance webhook-to-webhook calls were found in the 2026-05-01 audit.
- QA/build staging flow: Workflow Builder stages generated `[QA STAGING]` workflows in Kai n8n → QA Agent validates staged workflow JSON and test evidence → QA emails Kai report/final draft → only after Kai approval should a workflow be patched/copied into the client n8n.

## Live Test Snapshot — 2026-05-01
- Full synthetic single-client rehearsal run with slug `qa-rehearsal-20260501175136`.
- PASS: fake prospect created, Scoping Agent generated validated scope, Scope Approval generated proposal, Onboarding created Airtable Client + ClickUp folder `90149070509`, fake client moved through `build.ready` to `live`, Status Update Agent sent weekly status flow (`execution 2958`), Outreach reply-processing smoke completed (`execution 2960`), and Outreach remained active after temporary smoke webhook removal.
- Cleanup issue found: old rehearsal cleanup tried to set Prospects `project_status=test-complete`, which is not a valid Prospects select option. Fixed operationally by deleting synthetic test artifacts instead of leaving them marked complete.
- Cleanup completed for the 2026-05-01 rehearsal: deleted Prospect `recGdV7j1XrXZbJEJ`, Client `recnylxG8QKXiyxD3`, automation log `rectNvIWFgO89BM3U`, ClickUp folder `90149070509`, and outreach task `86b9q65rh`. Remaining Airtable records for that slug: 0 Clients, 0 Prospects.
- Cleanup completed for older `qa-rehearsal-*` artifacts from 2026-04-30: deleted 3 Clients, 3 automation logs, and 3 ClickUp folders. Remaining `qa-rehearsal-*` Airtable Clients/Prospects/logs: 0/0/0.
- Focused Scoping Notifier → Scoping Agent sub-workflow test run with slug `qa-subflow-20260501175443`: PASS. Browser-style `/trigger-scoping` entry invoked Scoping Agent through native Execute Sub-workflow, Prospect reached `scope_review`, and the synthetic Prospect was deleted immediately.
- Full 18-workflow health audit after patches: 15 active workflows, 3 inactive by design (`Reporting Agent`, `Workflow Builder Agent`, `QA Agent`), no broken connections, no activation-state mismatches, no stale same-instance webhook calls, and two expected sub-workflow links present.
...
| OpenRouter API | Workflow-specific Chinese models | — | n8n AI calls via OpenRouter HTTP or native `lmChatOpenRouter` nodes |
...
|------|-------|
| Provider | OpenRouter |
| Models | `qwen/qwen3.6-flash`, `qwen/qwen3.5-plus-20260420`, `qwen/qwen3.6-max-preview`, `~moonshotai/kimi-latest`, `deepseek/deepseek-v4-pro` |
| Max tokens | Workflow-specific: 350-4000 |
| Credential | `OpenRouter account` (`openRouterApi`, ID `hz6Vtt70sQ6Td1GQ`) |
| Endpoint | `https://openrouter.ai/api/v1/chat/completions` |
| Response parser | `choices[0].message.content` with legacy Anthropic fallbacks where useful |
| Reasoning control | HTTP AI calls send `reasoning: { effort: "none", exclude: true }` |

### Live OpenRouter model map — 2026-04-30
| Workflow | AI model | Budget fallback |
|----------|----------|-----------------|
| Website Chatbot | `qwen/qwen3.6-flash` | Keep current |
| Typeform Lead Qualification | `qwen/qwen3.6-flash` | Keep current |
| Status Update Agent | `qwen/qwen3.6-flash` | Keep current |
| Reporting Agent | `qwen/qwen3.5-plus-20260420` | `deepseek/deepseek-v4-flash` |
| Outreach Agent | `~moonshotai/kimi-latest` | `deepseek/deepseek-v4-flash` |
| Referral Trigger Agent | `~moonshotai/kimi-latest` | `deepseek/deepseek-v4-flash` |
| Scoping Agent | `qwen/qwen3.6-max-preview` | `deepseek/deepseek-v4-pro` |
| Scope Approval | `~moonshotai/kimi-latest` | `deepseek/deepseek-v4-pro` |
| Workflow Builder Agent | `deepseek/deepseek-v4-pro` | Keep current |
...
> Typeform thank-you button redirects to Calendly with `name={{field:full_name}}`, `email={{field:email}}`, and `a1=` populated with a fixed meeting-prep summary from `business_name`, `industry`, `team_size`, `pain_point`, and `hours_lost`. Verified 2026-05-07; webhook `pa-n8n-intake` remained enabled.
...
| email | email | |
| linkedin_url | url | |
| apollo_person_id | singleLineText | Apollo person ID for pre-reveal deduplication; added 2026-05-05 |
...
| reply_type | singleSelect: interested, not_now, wrong_person, unsubscribe, negative | Outreach reply quality classifier; added 2026-05-05 |
...
| [PA] Scoping Notifier | `nXXsF4E1BPWIS62r` | 17 | POST /scoping-notifier-airtable + hourly fallback + GET /trigger-scoping | 🟢 Active — patched 2026-05-08 so browser handoff marks `scoping_notified_at`; patched 2026-05-01 so browser/Airtable handoff calls Scoping Agent via Execute Sub-workflow instead of webhook-to-webhook |
...
12. Generate Email via Claude (HTTP POST → OpenRouter `qwen/qwen3.6-flash`)
...
13. Generate Report via Claude (OpenRouter `qwen/qwen3.5-plus-20260420` model node)
...
| Langchain AI node fails with HTTP Header Auth credential | `Credentials of type anthropicApi are not supported` | n8n Langchain AI nodes need provider-native credentials; HTTP Header Auth credentials are not accepted by model nodes | Launch mode uses either HTTP Request nodes to OpenRouter chat completions or native `lmChatOpenRouter` nodes with the `OpenRouter account` credential. Parse HTTP responses via `choices[0].message.content`. |
...
| Lead Generation duplicate-heavy reveal credits | Medium | RESOLVED 2026-05-05 - added `apollo_person_id` field, pre-reveal Apollo ID/email/LinkedIn dedup, backfilled 8 duplicate records from execution 3358, and added duplicate backfill branch for older records discovered after reveal | Haris |
...
| Outreach blank email send risk | Medium | RESOLVED 2026-05-05 - fixed malformed OpenRouter prompt expression that produced NaN prompt pollution and added Parse Email Sequence guard so blank/invalid AI output errors before SMTP send | Haris |
...
- [ ] **KAI/Haris:** Complete step-by-step full-system E2E rehearsal, one stage at a time: lead capture/source -> qualification -> scoping -> proposal -> onboarding -> credential handoff -> build staging -> QA -> activation/live -> status/update -> reply/referral checks. Start next session with Step 1 only and verify data after each stage.
...
## Session Handoff - 2026-05-05 (Lead Gen + Outreach Hardening, E2E Next)
**Worked by:** Haris + Codex
**Duration:** live workflow debugging + targeted patches

### What was completed
- Patched `[PA] Lead Generation`: expanded single-word health/wellness Apollo rotation; fixed stale connection that caused execution 3355 to error after writing 10 leads; added Airtable `apollo_person_id`; changed pre-reveal dedup to check Apollo ID, LinkedIn URL, and email.
- Investigated execution 3358: 10 Apollo reveals consumed, 2 new prospects written, 8 duplicates skipped only after reveal because older records lacked Apollo IDs. Backfilled all 8 duplicate records with `apollo_person_id` and added a duplicate backfill branch so older records discovered after reveal get Apollo IDs patched automatically.
- Patched `[PA] Outreach Agent`: added Airtable `reply_type`; reply detection now classifies replies as `interested`, `not_now`, `wrong_person`, `unsubscribe`, or `negative`; prompt now uses pain-angle sequencing across Email 1/2/3 and keeps Email 3 as a low-pressure call CTA.
- Fixed outreach prompt expression bug found during smoke test: malformed leading unary plus caused OpenRouter prompt pollution and blank emails. `Parse Email Sequence` now throws before SMTP send if generated email JSON is blank/invalid.
- Confirmed Calendly button remains in Email 2/3 HTML by Kai request.
- Smoke-tested Apollo fetch with keyword `spa`: 933 entries returned, relevant spa/medspa samples.
- Smoke-tested Outreach safely with a Kai-addressed temporary prospect: second run generated non-empty Email 1/2/3 and sent one test email to kai@phoenixautomation.ai.

### What is in progress (not finished)
- Full system E2E rehearsal requested next, one step at a time. Do not run the whole chain blindly; verify data state after each stage before moving forward.

### Blockers for next session
- None for Lead Generation or Outreach. Payment confirmation remains manual until Stripe webhook is built.

### Next person should start with
1. Read PROJECT_OVERVIEW.md in full.
2. Start a complete E2E rehearsal one stage at a time. Begin with Step 1 only: create or source a controlled test prospect and verify Airtable fields before triggering any downstream workflow.
3. After each stage, inspect Airtable/n8n/ClickUp state and ask/confirm before proceeding to the next stage.

### Files changed this session
- `PROJECT_OVERVIEW.md`
- Live n8n workflows via API: `[PA] Lead Generation` (`YO3f5CL9bYbLTBgw`), `[PA] Outreach Agent` (`Mib6RUtJ2IOaUZ4s`)
- Airtable Prospects schema: added/verified `apollo_person_id`, `reply_type`
...
### What was completed
- Updated live n8n AI workflows to workflow-specific Chinese models through OpenRouter using `.credentials.local` n8n API access.
- Verified old live AI model/API references (`z-ai/glm-4.5-air:free`, `claude-sonnet-4-6`, Anthropic API endpoint) are removed from live AI-bearing workflows.
- Documented the live model map and budget fallbacks in this overview.
- Discarded the local `kai-os` submodule change as requested; `kai-os` is clean.

### What is in progress (not finished)
- No live workflow execution/smoke test was run after the model swap to avoid sending emails or mutating client records.

### Blockers for next session
- Add payment/credits to OpenRouter before relying on paid-only models in production volume.
- Reporting Agent and Workflow Builder Agent remain inactive by design until eligible clients exist.

### Next person should start with
1. Add/confirm OpenRouter billing.
2. Run safe manual tests for Website Chatbot and Typeform first, then a controlled email workflow test.

### Files changed this session
- `PROJECT_OVERVIEW.md` — version 5.12, OpenRouter Chinese model map + budget fallbacks.
...
- **[2026-05-08]** - Apollo Lead Generation reliability hardening applied. Added retry/backoff to Apollo and Airtable HTTP nodes, `continueOnFail` plus error routing for the Apollo reveal step, an empty-seen-set fallback when pre-reveal Airtable dedup lookup fails, escaped email formulas for post-reveal dedup, paired/key-based prospect matching in `Dedup and Prepare1`, post-success keyword rotation, success/error status cleanup in automation logs, and removed the stale `advance-page-node` connection stub. Workflow readback confirmed active, new guard nodes present, retries set, and all Code nodes parse cleanly.

- **[2026-05-08]** - Apollo Lead Generation mismatched-org field leak fixed. `Normalize Revealed Prospects` now ignores revealed organization industry and employee count when Apollo bulk reveal returns a different organization than the vetted pre-reveal candidate. If the candidate industry is blank but the vetted company/title clearly indicates fitness/wellness, it infers `health, wellness & fitness`; otherwise it leaves industry blank rather than borrowing a mismatched employer value. Existing R3VIVE/Oswaldo row `recIhcNKO3M9tprBu` was repaired to `industry=health, wellness & fitness`, `team_size=10`.
...
- **[2026-05-07]** - Typeform to Calendly handoff improved. The live Typeform thank-you button now redirects to Calendly with Typeform recall tokens for `full_name` and `email`, plus Calendly custom answer `a1` as a fixed meeting-prep summary (`business_name`, `industry`, `team_size`, `pain_point`, `hours_lost`). Readback confirmed the redirect URL and that webhook `pa-n8n-intake` remains enabled.

- **[2026-05-07]** - Typeform Lead Qualification Airtable write logic fixed. Recent execution inspection showed Typeform sent `full_name`, but `Check Duplicate` replaced the item with Airtable search output before `Write to Airtable`, causing blank creates; the final score update then omitted `prospect_name`. Patched duplicate formula, create/update payloads, and backfilled matched records `recCXJoVP3SRgxfvv` and `recjoyfyEJFOQFGLf`. Live synthetic smoke was not run because it would create a production prospect; next manual Typeform submission should verify end-to-end.
